...
int Evaluate(int &clearScore, const AI_Param &ai_param,
             const GameField &last_pool, const GameField &pool, int cur_num,
             int curdepth, int total_clear_att, int total_clears, int clear_att,
  ...
  { // pool
    ...
    for (int x = 0; x < pool_w; ++x) {
      ...
      for (int y = std::min(min_y[x] + 1,
                            std::max(min_y[x - 1] + 6, min_y[x + 1] + 6));
           y <= pool_h; ++y, last = next) {
        if ((_pool.row[y] & (1 << x)) == 0) {
      ...
      if (cur_num == GEMTYPE_T && wallkick_spin && clears > 0 &&
        ...
        if (clears >= 3) {
          if (clear_att >= clears * 2) { // T3
            cs -=
                int(warning_factor * (ai_param.tspin3 * 8) + ai_param.hole * 2);
          }
        } else if (clears >= 2) {
          if (clear_att >= clears * 2) { // T2
            cs -= int(warning_factor *
                      (ai_param.tspin * 5 + ai_param.open_hole / 2));
          }
        } else if (wallkick_spin == 1) { // T1
          cs -= int(warning_factor *
                    (ai_param.tspin * 1 + ai_param.open_hole / 2));
      ...
      if (1)
        if (clears > 0 && upcomeAtt >= 4 && ai_param.upcomeAtt > 0) {
          ...
          if (avg - (12 + upcomeAtt) * pool_w > 0 && cur_s + cs < 0)
            s -= (cur_s + cs) * (avg - (12 + upcomeAtt) * pool_w) *
                 ai_param.upcomeAtt / pool_w / 10 / 20;
        }
#endif
      score += s;
    }
    ...
    if (softdropEnable() && maxy_index > 0 && maxy_index < pool_w - 1 &&
      ...
      if (min_y[maxy_index - 1] == min_y[maxy_index + 1] &&
        ...
        if (maxy_index > 1 &&
          ...
          ++cnt;
        if (maxy_index < pool_w - 2 &&
            min_y[maxy_index + 2] >= min_y[maxy_index + 1] - 2)
          ++cnt;
        if (cnt > 0) {
          score -= int(warning_factor * ai_param.tspin);
          if ((~pool.row[ybeg] & pool.m_w_mask) ==
              (1 << maxy_index)) { // T1基础
            score -= int(warning_factor * ai_param.tspin);
            if ((~pool.row[ybeg - 1] & pool.m_w_mask) ==
                (7 << (maxy_index - 1))) { // 可T2完美坑
        ...
        if (((row_y[3] >> (x + 1)) & (7)) == 1 /*100*/) { // 上图情况
          ...
          if (min_y[x + 1] > y) { // 洞判定
            if (x_holes[y - 1] > 0 || x_holes[y + 1] > 0 || x_holes[y] > 1 ||
                x_op_holes[y - 1] > 0 || x_op_holes[y + 1] > 0 ||
                x_op_holes[y] > 0) {
              continue;
            }
          } else {
          ...
          if (y <= pool_h - 3 && (pool.row[y + 3] & (1 << x)) == 0) {
            ...
            if ((r & (r - 1)) == 0) {
              score -= int(warning_factor *
                           (ai_param.tspin * 4 + ai_param.open_hole));
            }
          }
          {
          ...
          if (pool.row[y - 3] & (1 << (x + 1))) {
            if (t_dis < 7) {
              score -= int(warning_factor * (ai_param.tspin3 * 1) +
                           ai_param.hole * 2);
  ...
  if (USE4W)
    if (ai_param.strategy_4w > 0 && total_clears < 1) {
      ...
      if (maxy_4w - maxy_4w_combo < 9 &&
          pool_hole_score > ai_param.hole * (maxy_4w - maxy_4w_combo) / 2) {
        maxy_4w = -10;
      }
      ...
      if (maxy_4w > 8) {
        ...
        if (!has_hole) {
          ...
          if (sum == 3 || sum == 0 || sum == 4) {
            ...
            s += ai_param.strategy_4w * (hv) +
                 (ai_param.hole * 2 + ai_param.tspin * 4);
            if (sum > 0) {
  ...
  bool operator<(const MovsState &m) const {
    ...
    if (ai_settings[player].strategy_4w) {
      ...
      if (ai_settings[player].combo) {
        ...
        if (max_combo > combo && m.max_combo > m.combo &&
            (m.max_combo > 4 * 32 || max_combo > 4 * 32)) {
...
struct GameState {
  ...
  GameState(uint64 _hash, signed short _hold, signed short _att,
            signed short _clear, signed short _combo, signed short _b2b)
  ...
  bool operator<(const GameState &gs) const {
    ...
    if (att != gs.att)
      return att < gs.att;
    if (clear != gs.clear)
      return clear < gs.clear;
    if (combo != gs.combo)
      return combo < gs.combo;
    if (b2b != gs.b2b)
      return b2b < gs.b2b;
    return false;
  }
...
#define BEG_ADD_Y 1
MovingSimple AISearch(AI_Param ai_param, const GameField &pool, int hold,
                      Gem cur, int x, int y, const std::vector<Gem> &next,
                      bool canhold, int upcomeAtt, int maxDeep, int &searchDeep,
  ...
  if (pool.combo > 0 && (pool.row[10] || pool.combo > 1))
    ai_param.strategy_4w = 0;
  if (ai_param.hole < 0)
    ai_param.hole = 0;
  ...
  if (level <= 0)
    ...
    maxDeep = std::min(level, 6); // TODO: max deep
                                  // else maxDeep = level;
#else
...

  {
    ...
    for (MovingList::iterator it = movs.begin(); it != movs.end(); ++it) {
      ...
      wallkick_spin = ms.pool_last.WallKickValue(cur.num, (*it).x, (*it).y,
                                                 (*it).spin, wallkick_spin);
      ...
      if (clear > 0) {
        ms.combo =
            _pool.combo * combo_step_max + combo_step_max; // + 1 - clear;
      ...
      ms.first = *it;
      ms.first.score2 = 0;
      ms.first.score = Evaluate(ms.first.score2, ai_param, pool, ms.pool_last,
                                cur.num, 0, ms.att, ms.clear, att, clear,
                                wallkick_spin, _pool.combo, t_dis, upcomeAtt);
      if (wallkick_spin == 0 && it->wallkick_spin)
        ms.first.score += 1;
      que.push_back();
    }
  }
  if (canhold && !hold &&
      (pool.m_hold == 0 && !next.empty() &&
           !pool.isCollide(gem_beg_x, gem_beg_y, getGem(next[0].num, 0)) ||
    ...
    if (cur_num != cur.num) {
      ...
      for (MovingList::iterator it = movs.begin(); it != movs.end(); ++it) {
        ...
        ms.pool_last = _pool;
        signed char wallkick_spin = it->wallkick_spin;
        wallkick_spin = ms.pool_last.WallKickValue(cur_num, (*it).x, (*it).y,
                                                   (*it).spin, wallkick_spin);
        ...
        ms.max_att = att;
        ms.max_combo = ms.combo; // ms_last.max_combo + getComboAttack(
                                 // ms.pool_last.combo );
        ms.first = *it;
        ms.first.score2 = 0;
        ms.first.score = Evaluate(ms.first.score2, ai_param, pool, ms.pool_last,
  ...
  searchDeep = 1;
  for (int depth = 0; search_nodes < max_search_nodes && depth < maxDeep;
       searchDeep = ++depth) { // d < maxDeep
    std::swap(pq_last, pq);
    ...
    if (depth > 7)
      search_wide = sw_map[level][7];
    else
      search_wide = sw_map[level][depth];
#else
    int sw_map[16][8] = {
    ...
    int search_wide = 0;
    if (depth > 7)
      search_wide = sw_map[0][7];
    else
      search_wide = sw_map[0][depth];
    int search_base_width = sw_map[0][0];
    ;
#endif
    int seach_select_best =
        std::min(search_wide - search_wide / 4, search_base_width);
    if (level <= 3) {
      seach_select_best = search_wide - search_wide / 4;
    }
    if (seach_select_best < search_base_width) {
      seach_select_best =
          std::min(search_base_width, std::max(15, search_wide));
    }
    pq->clear();
    int max_combo = 3;
    int max_search_score = pq_last->back().first.score;
    {
      for (int s = pq_last->size(), i = s / 2; i < s; ++i) {
        max_search_score =
            std::max(max_search_score, pq_last->queue[i].first.score);
      }
      max_search_score =
          (max_search_score * 2 + pq_last->front().first.score) / 3;
    }
    std::set<GameState> gsSet;
    for (int pqmax_size = (int)pq_last->size(), pq_size = (int)pq_last->size(),
      ...
      if (pq_size != pqmax_size &&
          ms_last.first.score > 50000000) { // 超高分剪枝
        break;
      }
      ...
      int t_dis = 14;
      int d_pos = depth;
      if (next_add && ms_last.pool_last.m_hold)
        d_pos = depth + 1;
      if (d_pos >= next.size()) {
        pq->push(ms_last);
        continue;
      ...
      if (movs.empty()) {
        ...
        for (size_t i = 0; i < movs.size(); ++i) {
          ...
          MovsState &ms = p.append();
          {
            ...
            wallkick_spin = ms.pool_last.WallKickValue(
                cur_num, movs[i].x, movs[i].y, movs[i].spin, wallkick_spin);
            ms.pool_last.paste(movs[i].x, movs[i].y,
                               getGem(cur_num, movs[i].spin));
            int clear = ms.pool_last.clearLines(wallkick_spin);
            int att = ms.pool_last.getAttack(clear, wallkick_spin);
            ms.player = player;
            ...
            ms.first.score =
                Evaluate(ms.first.score2, ai_param, pool, ms.pool_last, cur_num,
                         depth + 1, ms.att, ms.clear, att, clear, wallkick_spin,
                         ms_last.pool_last.combo, t_dis, ms_last.upcomeAtt);
            if (wallkick_spin == 0 && movs[i].wallkick_spin)
              ms.first.score += 1;
          }
          p.push_back();
        }
      ...
      if (canhold && depth + next_add < next.size()) {
        ...
        if (ms_last.pool_last.m_hold != next[d_pos].num) {
          ...
          if (BEG_ADD_Y && ms_last.upcomeAtt < 0)
            GenMoving(ms_last.pool_last, movs, getGem(cur_num, 0),
                      AI::gem_beg_x, AI::gem_beg_y - ms_last.upcomeAtt, 1);
          else
            GenMoving(ms_last.pool_last, movs, getGem(cur_num, 0),
                      AI::gem_beg_x, AI::gem_beg_y, 1);
          if (movs.empty()) {
            MovsState ms = ms_last;
            ms.first.score += 100000000;
            pq->push(ms);
            ...
            for (size_t i = 0; i < movs.size(); ++i) {
              ...
              MovsState &ms = p.append();
              {
                ...
                ms.first.score2 = ms_last.first.score2;
                ms.first.score = Evaluate(
                    ms.first.score2, ai_param, pool, ms.pool_last, cur_num,
                    depth + 1, ms.att, ms.clear, att, clear, wallkick_spin,
                    ms_last.pool_last.combo, t_dis, ms_last.upcomeAtt);
                if (wallkick_spin == 0 && movs[i].wallkick_spin)
                  ms.first.score += 1;
  ...
  if (ai_settings[player].hash && canhold &&
      search_nodes < max_search_nodes) { // extra search
    std::swap(pq_last, pq);
    pq->clear();
    ...
    if (ai_settings[player].hash)
      sw_map = sw_map2;
    int search_base_width = sw_map[level][0];
    int search_wide = 1000;
    if (depth > 7)
      search_wide = sw_map[level][7];
    else
...
struct AI_THREAD_PARAM {
  ...
  int level;
  int player;
  AI_THREAD_PARAM(TetrisAI_t _func, Moving &_ret_mov, int &_flag,
                  const AI_Param &_ai_param, const GameField &_pool, int _hold,
                  Gem _cur, int _x, int _y, const std::vector<Gem> &_next,
                  bool _canhold, int _upcomeAtt, int _maxDeep, int &_searchDeep,
                  int _level, int _player) {
    func = _func;
    ret_mov = &_ret_mov;
    flag = &_flag;
    ai_param = _ai_param;
...
void AI_Thread(void *lpParam) {
  ...
  *p->flag = 1;
  ...
  AI::MovingSimple best = AISearch(p->ai_param, p->pool, p->hold, p->cur, p->x,
                                   p->y, p->next, p->canhold, p->upcomeAtt,
                                   p->maxDeep, searchDeep, p->level, p->player);
  AI::Moving mov;
  ...
  if (best.x != AI::MovingSimple::INVALID_POS) {
    ...
    for (size_t i = 0; i < movs.size(); ++i) {
      if (movs[i].x == best.x && movs[i].y == best.y &&
          movs[i].spin == best.spin) {
      ...
      } else if (cur.num == GEMTYPE_I || cur.num == GEMTYPE_Z ||
                 cur.num == GEMTYPE_S) {
...
int RunAI(Moving &ret_mov, int &flag, const AI_Param &ai_param,
          const GameField &pool, int hold, Gem cur, int x, int y,
          const std::vector<Gem> &next, bool canhold, int upcomeAtt,
          int maxDeep, int &searchDeep, int level, int player) {
  flag = 0;
  _beginthread(AI_Thread, 0,
               new AI_THREAD_PARAM(NULL, ret_mov, flag, ai_param, pool, hold,
                                   cur, x, y, next, canhold, upcomeAtt, maxDeep,
                                   searchDeep, level, player));
  return 0;
}
  ...
  *p->flag = 1;
  {
    ...
    for (int i = 0; i < g_combo_attack.size(); ++i) {
      ...
      if (i + 1 == g_combo_attack.size())
        comboTable[i + 1] = -1;
    }

    char *pOutStr = p->func(
        overfield, field, gamefield.width(), gamefield.height(), gamefield.b2b,
        gamefield.combo, next, gemMap[gamefield.m_hold], !p->hold,
        gemMap[p->cur.num], p->x, p->y, p->cur.spin, true, spin180Enable(),
        p->upcomeAtt, comboTable, p->maxDeep, p->level, p->player);
...
int RunAIDll(TetrisAI_t func, Moving &ret_mov, int &flag,
             const AI_Param &ai_param, const GameField &pool, int hold, Gem cur,
             int x, int y, const std::vector<Gem> &next, bool canhold,
             int upcomeAtt, int maxDeep, int &searchDeep, int level,
             int player) {
  flag = 0;
  _beginthread(AI_Thread_Dll, 0,
               new AI_THREAD_PARAM(func, ret_mov, flag, ai_param, pool, hold,
                                   cur, x, y, next, canhold, upcomeAtt, maxDeep,
                                   searchDeep, level, player));
  return 0;
}
